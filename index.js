// Consts
const apikey = "95f6a9836d787c84b3e39db3de18d7fb";
...
function searchMovies() {
    ...
    const searchUrl = `${apiEndpoint}/search/movie?api_key=${apikey}&query=${query}`;
...
function displaySearchResults(movies) {
    ...
    if (movies.length === 0) {
        moviesCont.innerHTML = "<h3>No results found</h3>";
        return;
    }

    const searchResultsHTML = movies
        .map(movie => `
            <div class="movie-item">
                <img class="movie-item-img" src="${imgPath}${movie.backdrop_path}" alt="${movie.title}" />
                <h3>${movie.title}</h3>
            </div>
        `)
        .join("");

    moviesCont.innerHTML = `<h2>Search Results</h2><div class="movies-row">${searchResultsHTML}</div>`;
}
